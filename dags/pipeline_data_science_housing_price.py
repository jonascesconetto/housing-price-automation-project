...
def _load_dataset_properties_to_staging():
    ...
    df_ = pd.read_csv(
      dags_path+"/data/properties_2016.csv", nrows=10000
    )
...
def _load_dataset_train_to_staging():
    ...
    df_ = pd.read_csv(
      dags_path+"/data/train_2016_v2.csv", nrows=10000
    )
...
def _join_datasets():
    ...
    df_final.to_parquet(
        staging_area+"/zillow_dataset.parquet", index=False
    )
...
def _check_remove_duplicated_rows():
    ...
    df_zillow.drop_duplicates(
        subset="parcelid", keep="first", inplace=True
    )
...
def _drop_columns_percent_missing_values(**kwargs):
    ...
    missing_var = [
        var for var in df_zillow.columns if df_zillow[var].isnull().sum() > 0]
    ...
    columns_drop = [
        var for var in missing_var if df_zillow[var].isnull().sum() > limit]
...
def _transform_rescale_features():
    ...
    df_zillow.drop(
      columns=['assessmentyear', 'yearbuilt','transactiondate'], 
      axis=1, 
      inplace=True
    )
...
def _encode_categorical_valiables():
    ...
    categorical_variables = [
        var for var in df_zillow.columns if df_zillow[var].dtypes == 'O']
    ...
    for i in range(len(categorical_variables)):
        ...
        var_mappings = {index: label for index,
...
def _drop_repetitive_useless_data():
    ...
    df_zillow.drop(
        columns=['censustractandblock', 'propertycountylandusecode_labels', 'parcelid'], axis=1, inplace=True
    )
...
def _choose_best_model(ti):
    models = [
        "LinearRegression", "AdaBoostRegressor", "GradientBoostingRegressor", "DecisionTreeRegressor", "RandomForestRegressor"
    ]

    metricas = ti.xcom_pull(
        key='mean_abs_error', task_ids=[
            "train_model_regression_linear", "train_model_ada_boost_regressor", "train_model_gradient_boosting_regression", "train_model_decision_tree_regressor", "train_model_random_forest_regressor"
        ]
    )
    index_best_model = metricas.index(min(metricas))

    print('Melhor modelo: {}, Score: {}'.format(
        models[index_best_model], metricas[index_best_model]))
...
def _final_model_train_dump(ti):
    ...
    best_model = ti.xcom_pull(
        key='best_model', task_ids=["choose_best_model"]
    )
...
(
  ...
  check_remove_duplicated_rows_task
) >> (
  drop_columns_percent_missing_values_task
) >> (
  transform_rescale_features_task
) >> (
  fill_missing_values_task
) >> (
  encode_categorical_valiables_task
) >> (
  drop_repetitive_useless_data_task
) >> (
  preprocessing_separate_train_test_task
) >> (
  [
